...
humidity = 0.0       # current humidity
avgHumidity = 1.0    # the average temp of the day
...
def humidityNLG( data ):
    ...
    if data["intent"] == "Time":
        ...
        if dateDifference > 0:      # future
            ...
            if hour <= 9 and hour > 6:  # 6~9시 아침
                output = output + "아침 예상 습도는 "
            elif hour >= 11 and hour <= 14:  # 11~14시 점심
                output = output + "점심 예상 습도는 "
            elif hour >= 17 and hour <= 20:  # 17~20시 저녁
                output = output + "저녁 예상 습도는 "
            elif hour > 20 and hour < 24:  # 20~24시 밤
                output = output + "밤 예상 습도는 "
            elif hour >= 0 and hour <= 6:  # 0~6시 새벽
                output = output + "새벽 예상 습도는 "
            else:
                output = output + "예상 습도는 "  # default

        elif dateDifference < 0:    # past
            currentTimeFlag = 0
            pastTimeFlag = 1
            if hour <= 9 and hour > 6:              # 6~9시 아침
                output = output + "아침 습도는 "
            elif hour >= 11 and hour <= 14:         # 11~14시 점심
                output = output + "점심 습도는 "
            elif hour >= 17 and hour <= 20:         # 17~20시 저녁
                output = output + "저녁 습도는 "
            elif hour > 20 and hour < 24:           # 20~24시 밤
                output = output + "밤 습도는 "
            elif hour >= 0 and hour <= 6:           # 0~6시 새벽
                output = output + "새벽 습도는 "
            else:
                output = output + "습도는 "          # default

        else:                       # today !!
            if abs(timeDifference) < 2:             # right now
                currentTimeFlag = 1
                output = output + "현재 습도는 "
            else:
                if hour <= 9 and hour > 6:  # 6~9시 아침
                    output = output + "아침 습도는 "
                elif hour >= 11 and hour <= 14:  # 11~14시 점심
    ...
    if avgHumidityFlag == 1:
        output = output + str(avgHumidity) + "입니다."
    else:
        output = output + str(humidity) + "입니다."
